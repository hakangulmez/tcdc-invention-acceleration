#!/usr/bin/env python3
"""Add Sections 13-16 to the notebook."""
import json
import os
...
for _, row in new_gpts.iterrows():
    ...
    print(f"    Ω-shift: {row['omega_shift_description']}")
    print(f"    Source: {row['source']}")
    print()

# Show interval changes
print("=" * 70)
...
# Filter GPTs to post-1000 for the figure
gpt_post1000 = gpt_df[gpt_df['year'] >= 1000].copy()

# GDP growth before/after each GPT
...
gpt_post1000 = gpt_post1000.drop_duplicates(subset='year')  # Remove simultaneous GPTs for clarity
gpt_post1000['gdp_at_arrival'] = gpt_post1000['year'].apply(lambda y: float(interp_func(y)))

# Era colors
def era_color(year):
    if year < 1450: return '#8B4513'    # Medieval brown
    elif year < 1769: return '#DAA520'  # Renaissance gold
    elif year < 1882: return '#FF6347'  # Steam red
    elif year < 1971: return '#4169E1'  # Electric blue
    else: return '#9370DB'              # Digital purple
...
# GPT markers and labels
for _, gpt in gpt_post1000.iterrows():
    color = era_color(gpt['year'])
    ax.axvline(gpt['year'], color=color, lw=1.2, ls='--', alpha=0.6)
    ax.scatter(gpt['year'], gpt['gdp_at_arrival'], color=color, s=80, zorder=5, edgecolors='black', lw=0.5)

    # Smart label placement
    name_short = gpt['name'].split('(')[0].strip()[:25]
    y_pos = gpt['gdp_at_arrival'] * 1.5
    rotation = 45
    ax.annotate(name_short, (gpt['year'], gpt['gdp_at_arrival']),
                xytext=(5, 15), textcoords='offset points',
                fontsize=7, rotation=rotation, color=color, fontweight='bold',
...
ax.text(0.02, 0.02,
        ...
        'Note: Shaded regions show within-Ω periods;\\n'
...
# =========================================================================
# SECTION 15 — GDP GROWTH EVENT STUDY
# =========================================================================
new_cells.append(md_cell("""---
...
as they shift the technological frontier Ω and open new combinatorial possibilities."""))
...
# For each post-1500 GPT, build ±50 year growth window
event_windows = []
...
for _, gpt in gpt_post1500.iterrows():
    ...
    for rel_year in range(-50, 51):
        ...
        # Only use years within Maddison range
...
# Calculate annualized growth rates within each window
...
# Pre vs post boost for each GPT
print("\\n" + "=" * 70)
...
# T-test: is boost significantly different from 0?
...
fig.suptitle('Figure 7: GDP Growth Around GPT Arrivals — Event Study', fontsize=14, fontweight='bold')

# Left: average path
ax = axes[0]
ax.fill_between(avg_path['rel_year'], avg_path['ci_lo'], avg_path['ci_hi'], alpha=0.3, color='steelblue')
...
ax.text(5, ax.get_ylim()[1]*0.85, f'Post-GPT boost:\\n+{boost:.2f} pp', fontsize=11, color='darkgreen',
        bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.5))

# Right: individual GPT paths
ax2 = axes[1]
...
for i, (_, gpt) in enumerate(gpt_post1500.iterrows()):
    ...
    if len(gpt_data) > 20:
        ...
        ax2.plot(path.index, path.values, alpha=0.5, lw=1.5, color=colors2[i], label=gpt['name'][:25])
...
plt.close()
print(f"Saved: figures/fig7_event_study.png/.pdf")
"""))

new_cells.append(code_cell("""# === Figure 8: Schumpeterian S-Curve — Diminishing Returns Within Ω ===
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from scipy.interpolate import interp1d
...
# Create annual interpolation
interp_func = interp1d(maddison_df['year'], maddison_df['gdppc_2011usd'],
                        kind='linear', fill_value='extrapolate')

fig, axes = plt.subplots(1, 3, figsize=(18, 6))
fig.suptitle('Figure 8: Schumpeterian Dynamics — Diminishing Returns Within Each Ω',
...
for ax, (era_name, t_start, t_end, color) in zip(axes, eras):
    # Create annual data for this era
    ...
    # Smooth with rolling average (10-year window)
    ...
    ax.plot(t_rel[valid], growth_smooth[valid], color=color, lw=2.5, alpha=0.8, zorder=3, label='10yr MA')
...
# =========================================================================
# SECTION 16 — GDP BOOST → NEXT INTERVAL REGRESSION
# =========================================================================
new_cells.append(md_cell("""---
...
new_cells.append(code_cell("""# === Section 16A: Construct Boost Variable ===
import numpy as np
import pandas as pd
from scipy.interpolate import interp1d
from scipy import stats
...
# For each post-1500 GPT: calculate 30-year boost
gpt_post1500 = gpt_df[(gpt_df['year'] >= 1500) & (gpt_df['year'] <= 2000)].copy()
...
for idx in range(len(gpt_post1500)):
    ...
    # Pre-GPT growth (30 years before)
    ...
    # Post-GPT growth (30 years after)
...
if beta_boost < 0:
    ...
    print(f"          → Positive coefficient (may reflect data limitations)")
print(f"  Spec A R²: {model_a.rsquared:.3f}")
print(f"  Spec B R²: {model_b.rsquared:.3f}")
"""))
...
# OLS line + CI
ax.plot(x_range, y_pred, 'r-', lw=2, label=f'OLS: β={model.params.iloc[1]:.3f} (p={model.pvalues.iloc[1]:.3f})')
...
ax.text(0.02, 0.98, f'R² = {r2:.3f}\\nN = {len(reg_df)}\\n'
        f'β(boost) = {model.params.iloc[1]:.3f}',
